...
const hash = await crypto.subtle.digest("SHA-256", new TextEncoder().encode(pepper + salt + password))
...
const verifyPassword = async (password: string, encodedSalt: string, encodedHash: string) => {
    ...
    const newHash = await crypto.subtle.digest("SHA-256", new TextEncoder().encode(pepper + salt + password))
...
const ids = scanData.Items?.map((item) => item.id?.N as string).map((id) => parseInt(id)) ?? []
