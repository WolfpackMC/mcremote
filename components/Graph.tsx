...
const Graph = () => {
    ...
    return <>
        ...
        onMouseEnter={() => setPaused(true)} onMouseLeave={() => setPaused(false)} ref={graphRef} className="w-full h-full" />
        <div className="top-0 left-0 m-2">
            {labels.map((label) => <button onClick={
                () => {
            ...
            } className={`bg-zinc-900/75 rounded-xl transition-colors p-2 m-1 ${visibility[labels.indexOf(label)] ? "text-zinc-100" : "text-zinc-500"}`}
